...
const RelatedProduct = () => {
    ...
    const products = useSelector((state) => {
        const product = state.productsReducer.products.find(
            (product) => product.id === productId
        );

        return state.productsReducer.products.filter(
            (prod) => product.related.indexOf(prod.id) >= 0
        );
    });
    ...
    return (
        ...
        >
            <div className="absolute -top-8 left-0 text-green-300 font-bold">
                ...
                <a
                    target="_blank"
                    href="https://github.com/dotuan9x/micro-frontends/tree/master/react-redux/products"
                    rel="noreferrer"
                >
                    products
                </a>
            ...
            <ul className="flex flex-col">
                {products.map((product) => {
                    return (
                        <li
                            key={product.id}
                            className="cursor-pointer opacity-100 hover:opacity-80"
                        >
                            <img
                                className="w-36"
                                onClick={() =>
                                    dispatch(updateProductId(product.id))
                                }
                                alt=""
                                src={product.image}
                            />
